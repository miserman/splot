...
***
The rest of the guide may be most useful if you get R set up and started, then follow along.  
Try entering the commands in the colorful boxes into an R console; the expected result of those commands is below each colorful box (with lines preceded by ##).


# Working with data
Data are single numbers (e.g., `100`) or strings (e.g., `'abc'`). These are generally contained in vectors (sets of values; e.g., `1, 2, 3`).

In R, vectors can be created with the `c()` function. For example:
```{r}
c(1,2,3)
```

You can also make a vector of sequential values with the `:` function:
```{r}
1:3
```

Multiple vectors can be represented as a matrix:
```{r}
matrix(0,2,3)
```
For the `matrix()` function, the first argument---the value entered before the first comma within the parentheses---dictates the value within each cell of the matrix, the second argument dictates the number of rows (horizontal lines), and the third argument dictates the number of columns (vertical lines).

To save a matrix, we can assign it a name with the `=` function (the `<-` function would also work):
```{r}
m = matrix(1:6,3)
```
Entering the name now prints the matrix:
```{r}
m
```
We can selected specific values within the matrix with the `[` function. For example, this will select the cell in the third row of the second column:
```{r}
m[3,2]
```
Datasets are generally represented as matrices with variables as columns. For example, we can add names to our matrix:
```{r}
colnames(m) = c('V1','V2')
m
```
Here, the `=` function is assigning names by way of the `colnames()` function, but `colnames()` can also display names:
```{r}
colnames(m)
```
Now that the `m` matrix's columns have names, we can select them by name:
```{r}
m[,'V1']
```
And we can do the same for rows:
```{r}
rownames(m) = c('R1','R2','R3')
m['R1',]
```
Printing the `m` matrix again will now display it and all of its column and row names:
```{r}
m
```
This is the way the Motor Trend car dataset (`mtcars`) is arranged, which is included in base R.  
Each row is a car, and each column is a feature of that car:
```{r}
mtcars
```
You can use the help function (`?`) followed by a function or dataset name to access its documentation (e.g., `?mtcars`).

# Visualizing data
`splot()` is the main function of the splot package. Its first argument is for variable names, and its second argument is for the dataset containing those variables.
...
splot(mpg ~ wt, mtcars)
```
Each dot represents a car, and its position is a combination of its MPG and weight; the higher it is vertically, the more miles it can go per gallon of gas, and the farther it is horizontally, the more it weighs (in tons).
...
Something we might do to improve this prediction (model fit) is to consider other variables. Weight seems to be closely related to MPG (going by the last plot), but maybe MPG depends on something else as well, such as the car's style of transmission (automatic versus manual). To look at this, we can add a splitting variable to the formula.

Splitting variables break the data up into groups based on their value. For example, this will separate cars that have an automatic transmission (0; green) from those that have a manual transmission (1; purple), and estimate a line for each group.
```{r, fig.height=4.5, fig.width=10, dev='CairoSVG'}
