...
- fill the area under each density lines with its color with
  density.fill = TRUE (default), adjust the fill’s opacity with
  density.opacity, and further control the densities with arguments in
  density.args (which replaces the old bw and adj arguments).
...
- handles mixed-type multiple ys by converting non-numeric variables to
  numeric when other numeric variables are included.
...
- assign colors in the scale of numeric values with splot.color’s grade
  argument.
...
- preserves color assignments when colorby has multiple arguments, and
  some levels of by are empty.
...
- more reliable connection between data and line colors, types, and
  widths.
...
- improved formula parsing; expressions should be better recognized as
  arguments.
...
- multiple variables entered as y can now be renamed/ordered with an mv
  entry in levels.
...
- standardized lty behavior across types, and prevented it from causing
  errors.
...
- added utility functions: splot.color to assign colors by group, and
  splot.bench to plot the run-time of expressions.
...
- bar and line plots no longer skip levels of x with only one level of
  by.
