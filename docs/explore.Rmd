...
#lpos sets the legend position when it isn't automatically placed out of the way.
splot(data[,metrics], lpos='topright')
```
```{r, fig.height=3, fig.width=10, dev='CairoSVG'}
...
#lty makes all of the lines solid, and mxl spreads the replies out a little.
splot(Canberra~reply*ParticipantID, su=Study==4, lim=F, lty=1, mxl=c(1,2))
```
...
splot(
  ...
  mv.scale=T, lpos='topright', levels=list(prompt=c(4,2,1,3))
)
```
